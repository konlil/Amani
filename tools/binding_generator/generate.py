...
def generate_ts_declarations(api, whitelist, output_path):
    ...
    lines.append("")
    lines.append("    // High-level helper API")
    lines.append("    function vec2(x?: number, y?: number): Vector2;")
    lines.append("    function vec3(x?: number, y?: number, z?: number): Vector3;")
    lines.append("    function color(r?: number, g?: number, b?: number, a?: number): Color;")
    lines.append("")
    lines.append("    /** Add a node to the scene root (uses call_deferred internally). */")
    lines.append("    function addToScene(node: Node): void;")
    lines.append("")
    lines.append("    interface PrimitiveOptions {")
    lines.append("        size?: number | Vector3;")
    lines.append("        color?: Color;")
    lines.append("        position?: Vector3;")
    lines.append("        name?: string;")
    lines.append("    }")
    lines.append("    /** Create a MeshInstance3D with a built-in mesh. type: 'box'|'sphere'|'cylinder'|'capsule'|'prism'|'triangle'|'plane'|'quad' */")
    lines.append("    function createPrimitive(type: string, opts?: PrimitiveOptions): MeshInstance3D;")
    lines.append("")
    lines.append("    interface SceneOptions {")
    lines.append("        background?: Color;")
    lines.append("        camera?: { position?: Vector3 };")
    lines.append("    }")
    lines.append("    /** Set up a basic 3D scene with camera, directional light, and environment. */")
    lines.append("    function setupScene(opts?: SceneOptions): { camera: Camera3D; light: DirectionalLight3D };")
    lines.append("")
    lines.append("    // Enum constants")
    lines.append("    const PRIMITIVE_TRIANGLES: number;")
    lines.append("    const SHADING_MODE_UNSHADED: number;")
    lines.append("    const BG_COLOR: number;")
    lines.append("")
    lines.append("}")
    lines.append("")
